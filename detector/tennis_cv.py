...
from common.detect_data import DetectData
...
class TennisColorDetector:
    ...
    def process(self, frame):
        ...
        # 开运算，先腐蚀再膨胀，用于去除小的噪点
        mask = cv2.morphologyEx(mask, cv2.MORPH_OPEN, kernel, iterations=2) # 1
        ma=mask
        # 闭运算，先膨胀再腐蚀，用于填充孔洞
        ...
        for cnt in contours:
            ...
            if (
                ...
                # data.append([x,y,w,h])
                data.append(DetectData(x,y,w,h))
...
def test():
    dector = TennisColorDetector([30, 70, 80], [50, 255, 255], min_area=300)
